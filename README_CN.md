...
> 基本准则:让数据库干数据库需要干的事,而不是通过orm

- 移除了驱动库依赖的，规避项目使用时需要download所有驱动库的问题(后续考虑在项目里tests+驱动库做为一个子包规避问题，同时保留内置测试)
- 移除部分依赖，实际无使用场景，如:github.com/syndtr/goleveldb
- 后续会不断取舍做减法，打磨顺手
